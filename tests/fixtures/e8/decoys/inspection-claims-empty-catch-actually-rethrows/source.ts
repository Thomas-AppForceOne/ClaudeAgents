 ...
 * decoy.
 *
 * The faked finding's evidencePointer cites line 14 and claims an empty
...
export function tryParse(raw: string): unknown {
  try {
    ...
    // Line 14 (per the decoy's evidencePointer) is the line above this
    // comment — `} catch (err) {`. The block logs + rethrows, NOT empty.
