 ...
 * - schema conformance: every event class the emitter can produce validates
 *   against run-trace-v1.json, both as the in-memory object and after
 *   round-tripping through the atomic on-disk write.
...
// Emits exactly one event of each known class, in a fixed order, so a single
// fixture can drive both the "all classes validate" and "sequence is gapless
// across mixed classes" checks. Keeping one emit per class here is the
// mechanism that ties this suite to KNOWN_EVENT_TYPES: a class added to the
// union without a line here fails the completeness assertion below.
function emitOneOfEach(emitter: TraceEmitter) {
  const events = [
    ...
    emitter.emitClarifierUserAction({
      action: 'evolved',
      round: 1,
...
describe('event_validates_against_schema — every known class', () => {
  ...
  it('the on-disk event files also validate (round-trip through atomicWriteFile)', () => {
    ...
    expect(files).toHaveLength(KNOWN_EVENT_TYPES.size);
...
describe('sequence_monotonic_gapless', () => {
  it('allocates strictly increasing, gapless, non-negative sequence numbers across mixed classes', () => {
    ...
    // The expected run is 0..n-1 over however many classes the fixture emits, so
    // adding a class extends the baseline automatically rather than silently
    // skewing a hard-coded literal.
