/**
 * Bundling-invariant test for `schemas/progress-v1.json`.
 *
 * Two assertions:
 * 1. The bundled-schemas module exports `progressV1` with the
...
const schemaPath = path.join(repoRoot, 'schemas', 'progress-v1.json');

describe('schemas-bundled — progress-v1', () => {
  it('exports progressV1 with the canonical $id', () => {
    ...
    expect((progressV1 as { $id?: string }).$id).toBe(
      'https://claudeagents.dev/schemas/progress-v1.json',
    );
  });
  ...
  it('the bundled copy deep-equals the on-disk schema', () => {
    ...
    expect(progressV1).toEqual(onDisk);
  });
  ...
  it('declares additionalProperties: false at the top level', () => {
    ...
    // tests in tests/schemas/progress-v1.test.ts.
    expect((progressV1 as { additionalProperties?: unknown }).additionalProperties).toBe(false);
  });
});
