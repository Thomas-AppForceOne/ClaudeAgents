/**
 * Bundling-invariant test for `schemas/telemetry-config-v1.json`.
 *
 * Two assertions:
 * 1. The bundled-schemas module exports `telemetryConfigV1` with the
...
const schemaPath = path.join(repoRoot, 'schemas', 'telemetry-config-v1.json');

describe('schemas-bundled — telemetry-config-v1', () => {
  it('exports telemetryConfigV1 with the canonical $id', () => {
    ...
    // the contract.
    expect(telemetryConfigV1).toBeDefined();
    expect((telemetryConfigV1 as { $id?: string }).$id).toBe(
      'https://claudeagents.dev/schemas/telemetry-config-v1.json',
    );
  });
  ...
  it('the bundled copy deep-equals the on-disk schema', () => {
    ...
    expect(telemetryConfigV1).toEqual(onDisk);
  });
  ...
  it('declares additionalProperties: false at the top level', () => {
    ...
    expect(
      (telemetryConfigV1 as { additionalProperties?: unknown }).additionalProperties,
    ).toBe(false);
  });
});
