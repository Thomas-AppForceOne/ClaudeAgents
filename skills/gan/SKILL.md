...
9. **Create the worktree.** Call `createRunWorkspace({ subject, runId })` — wraps the framework's `resolveWorkspace` (`src/config-server/storage/worktree-resolver.ts`, cases 1a / 1b / 1c). Act on the returned `{ worktreePath, branch, createdByGan, resolutionCase, mutated }` (`mutated` equals `createdByGan` — `true` for 1b/1c, `false` for 1a; it is the worktree-creation mutation signal and does not feed the snapshot-freshness re-snapshot rule, which is scoped to agents' configuration writes):
