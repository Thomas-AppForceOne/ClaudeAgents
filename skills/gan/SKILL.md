...
9. **Create the worktree.** Call `createRunWorkspace({ subject, runId })` — wraps the framework's `resolveWorkspace` (`src/config-server/storage/worktree-resolver.ts`, cases 1a / 1b / 1c). Act on the returned `{ worktreePath, branch, createdByGan, resolutionCase }`:
...
11. **Tear down and release the lock.** On **every** run-exit path — graceful completion, abort (user cancel, `validateAll` failure after the lock is held, `ConcurrentRunInProgress` is the one exception: nothing was acquired), or error (uncaught failure, loop-detection halt, recovery refusal) — the orchestrator marks the run terminal in `progress.json` (under `GAN_RUN_DIR`), removes the worktree filesystem (the run branch survives for inspection), and **then calls `releaseRunLock({ repoKey })`**. The release is the last write the orchestrator performs for the run. Release-on-exit cannot be deferred: the acquiring process is the long-lived config server, so a lock left held by it would never be self-healed as stale, and the next `/gan` on the same repo would be refused indefinitely. Release is idempotent — a double-release on an overlapping graceful-then-error path is safe.
