...
- **`evaluating`** — the orchestrator re-spawns the evaluator on the most recent generator-attempt commit (the one whose SHA the trace records for the recorded `currentSprint` / `currentAttempt`). If a `sprint-N-feedback-A.json` artefact is present but was being written when the run halted (a partial write detectable by malformed JSON), the orchestrator discards the partial and re-evaluates rather than trusting half-written feedback.
