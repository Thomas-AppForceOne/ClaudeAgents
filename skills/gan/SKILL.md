...
8. **Create the worktree.** Use `.gan-state/runs/<run-id>/worktree` (a run-state path). Record run metadata in `.gan-state/runs/<run-id>/progress.json`. The `<run-id>` follows the established `<YYYYMMDDTHHMMSS>-<4 hex>` form.
