...
5. **Re-lock at a new contract revision.** The orchestrator passes the audited draft to the re-lock helper (`relockContract`), which archives the prior canonical contract as a `.r{k}.json` sibling and atomically swaps the new draft into the canonical filename. The new revision becomes the authoritative contract for every downstream step in this attempt; the prior revisions remain on disk as operator-readable history. See the section below for the lifecycle and crash semantics.
