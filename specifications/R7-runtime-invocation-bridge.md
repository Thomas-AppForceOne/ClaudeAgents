...
- **Trace is non-empty after a run.** A fixture sprint driven through the orchestrator path emits at least one `agentAttempt` event per spawned agent attempt; the run's `trace/events/` is non-empty and the index resolves every event with gapless sequence numbers.
