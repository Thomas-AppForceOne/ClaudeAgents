...
- **Trace (`src/trace/`)**
  ...
  - `reconstructRevisionState({ runDir, contractRevision })` → **new** pure helper (introduced by E8, **not** an edit to the shipped whole-trace `reconstructRecoveryState`): filters `agentAttempt` events by their `contractRevision` field and returns the per-role tally for that revision only. This is what lets the orchestrator feed *revision-scoped* attempt counts to the shipped `checkSprintBudget`/`checkRoleCeiling` (E8 § "Bounding thrash"). It relies on `agentAttempt` carrying an optional `contractRevision` field — an **additive** field on `run-trace-v1`, edited in place per the schema-versioning ruling (roadmap § "Schema-versioning ruling"), the same way A1 added the `safetyHalt` class to `run-trace-v1`.
