...
- Two worktrees of the same repo resolve to the **same** `<module-state-root>/<repo-key>/docker/port-registry.json`; the second worktree's `PortRegistry` sees the first's allocation and picks a different host port — a regression test for M2's lines 146–149 guarantee, which fails under the pre-F8 per-worktree layout.
