...
**Update — user review in progress (2026-05-02).** Two of the eight open questions have been resolved during interactive review with the user; resolutions are captured inline in §6 with cross-references to the new spec revisions in §7 (items 21-30). One additional open question (OQ#9) was raised during review — platform priority — and resolved in the same pass.
...
1. **OQ#1 — F4/R5 trust-prompt UX exercise against three real PRs.** The roadmap's gating bullet ("exercise against three real PRs of varying shape") cannot be done in a desk audit. The interactive prompt itself is not yet shipped (R5 slice 5 is an E1 sprint slice). Confirm: defer the three-real-PR exercise to E1's first real-world PR review, or block Phase 3 on a synthetic exercise now? Recommend deferring; the prompt's text and routing is exercised by unit tests of `getTrustState()` already.
2. **OQ#2 — Banner text canonicalisation.** PROJECT_CONTEXT.md line 169 says the banner is "`# DRAFT — replace TODOs and remove this banner before committing.`" (longer). Code uses "`# DRAFT — replace TODOs before committing.`" (shorter, with a separate second line). F3, R4, R3 line 32, R3 line 65 are inconsistent. Decide which canonical wording is right and update everything (PROJECT_CONTEXT, all four specs, the constant in `src/config-server/scaffold-banner.ts`, and the test fixture under `tests/fixtures/stacks/invariant-stack-draft-banner/`).
3. **OQ#3 — F2 capability-binding deferral re-check.** F2 line 41 explicitly defers capability binding to the post-R audit. The audit's question: should we close the deferral now (no current code path violates it; no real-world surface yet exists) or wait until E1 lands and the orchestrator's caller graph is concrete? Recommend the latter — the answer is more meaningful when there's a real caller pattern.
4. **OQ#4 — User-tier file name (`user.md` vs `config.md`).** Spec says `config.md` (C3, C4, F4, U2); code uses `user.md`. The rename either way is one-line. Which way does the user want?
...
6. **OQ#6 — User-tier-forbidden enforcement.** C3 says user-tier `additionalContext` is a hard error at load. Code allows it (and a test at `tests/config-server/tools/writes.test.ts:472` expects success). Should the implementation start rejecting these (correct per the threat model), or should C3 be revised to permit them? Recommend implementing the rejection — it's the threat-model invariant.
7. **OQ#7 — F3 `path.no_escape` invariant deduplication.** Code ships two invariants checking the same fields (`path.no_escape` raises `InvariantViolation`; `path.escape` raises `PathEscape`). Pick one. Recommend keeping `path.escape` (the F2-cataloged code) and deleting `path.no_escape`.
8. **OQ#8 — F2 surface drift acceptance.** Should F2 absorb the five extra tools (`getMergedSplicePoints`, `getStackResolution`, `appendToOverlayField`, `removeFromOverlayField`, `trustList`) and three error codes (`NotImplemented`, `MalformedInput`, `CacheEnvConflict`)? Recommend yes — they're shipped and used.
...
4. **F3 `specifications/F3-schema-authority.md`, line 84 (`path.no_escape` row).** Either rename to `path.escape` (matching the F2 error code) or note that this invariant raises `PathEscape` rather than `InvariantViolation`. Coordinate with R1 invariant consolidation (delete `path.no_escape.ts`, keep `path.escape.ts`).
...
9. **C3 `specifications/C3-overlay-schema.md`, line 12.** Change `~/.claude/gan/config.md` to `~/.claude/gan/user.md` (or revise the implementation to use `config.md`; user must decide). Cascade the change to F4 line 148, U2 lines 17/87, C4 line 12.
10. **C3 `specifications/C3-overlay-schema.md`, lines 71-75 + R1.** Either implement user-tier-forbidden enforcement in `src/config-server/storage/overlay-loader.ts` (recommended) or revise C3 to drop the rule. The current state where the rule exists but is unenforced is the worst of both worlds.
...
16. **R3 `specifications/R3-cli-wrapper.md`, line 65 + R3 line 32 + R4 line 48 + F3 line 89 + PROJECT_CONTEXT line 169 + the `DRAFT_BANNER` constant in `src/config-server/scaffold-banner.ts`.** Pick one canonical banner text. Recommend `# DRAFT — replace TODOs before committing.` (current code value, shorter, period-terminated); update all other locations to match. Document the SECOND_LINE separately as a scaffold-only annotation.
...
End of audit. None of the findings block Phase 3 starting. Two of the nine open questions (OQ#5, OQ#9) have been resolved during user review on 2026-05-02; the remaining seven (OQ#1, OQ#2, OQ#3, OQ#4, OQ#6, OQ#7, OQ#8) await user decision. User approval of items 1-30 (or rejection with reason) closes the audit gate.
