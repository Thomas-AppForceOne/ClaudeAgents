...
The audit resolves drift **toward the runtime contract**: where the published schema declared a payload required (`value` / `state`) but the lax runtime did not enforce it, the runtime is tightened to match the schema rather than the schema loosened to match the runtime. Consequence — a deliberate, disclosed behaviour change to the write tools' input surface: `setOverlayField`, `appendToOverlayField`, `removeFromOverlayField`, `updateStackField`, `appendToStackField`, `removeFromStackField`, `setModuleState`, and `appendToModuleState` now reject a **missing** payload with a structured `MalformedInput` (carrying the offending `field`) instead of silently persisting `undefined` as the field value. `null` remains a valid payload (it is a legitimate value to store); only an absent or `undefined` payload is rejected. This narrows the accepted input set, so it is called out here explicitly rather than treated as a transparent "alignment": no internal caller relied on the old lax behaviour, and a schema-conformant external caller was already sending the payload, so the change is breakage-free in practice while closing a silent-corruption footgun.
