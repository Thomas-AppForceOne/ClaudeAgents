...
`.gan-state/` does **not** vanish, but it empties: F7 moves `runs/` out, and its paired spec **[F8](F8-centralized-module-state-store.md)** (same PR) moves `modules/` to a separate store — so after both, the project-local `.gan-state/` holds only a gan-created run-scoped `worktree/` (cases 1b/1c). Zone-3 cache (`.gan-cache/`) is unchanged. From F7 *alone*, module state would stay project-local per F1; relocating it is F8's job (out of F7's own scope — see "What F7 does not do").
...
- **(1b) Wrap the matching branch.** The current branch matches the task slug but the current directory is **not** a worktree dedicated to it (e.g. the task branch is checked out in the main checkout) → the orchestrator gives that existing branch its own run-scoped worktree (see *Freeing a branch for 1b* below).
...
**Freeing a branch for 1b.** A branch can be checked out in only one worktree, so before `git worktree add`-ing the (currently-occupied) task branch, the orchestrator frees it: it switches the current checkout to the repo's default branch (e.g. `develop`; a detached HEAD as a fallback if that branch is itself occupied), then adds the task branch into the run-scoped worktree. This **requires the current checkout's working tree to be clean** — if it is dirty, the run **refuses** with guidance (commit or stash the changes, or run from a dedicated worktree) rather than auto-stashing or detaching over uncommitted work. `git worktree add --force` (which would check the branch out in two worktrees and invite divergent commits) is never used.

**Worktrees created by gan are run-scoped and live in the project tree**, exactly as the current orchestrator places them: `<project>/.gan-state/runs/<run-id>/worktree/`, gitignored, ephemeral, removed by run cleanup (§4). This keeps the generator's code writes inside the workspace boundary — no permission prompts (§3) — and reuses proven, shipped worktree mechanics. The durable, task-named artifact is the **branch**: case 1c creates a branch named after the task and checks the run-scoped worktree out to it; case 1b reuses the matching branch the same way. (`--cleanup` removes such a branch only when it is merged, or with explicit confirmation — §4.) What survives a worktree's removal is the run *data*, which lives in the central store, not the worktree (§1).
...
- **`--cleanup`** always deletes the central-store run directory. For a gan-created workspace (`workspace.createdByGan` true) it is **merge-aware**: it removes the run-scoped worktree at `.gan-state/runs/<run-id>/worktree/`, then handles the task branch by merge status — if the branch is **merged** into its base/upstream, cleanup deletes it both locally and on the remote (when a tracking branch exists); if it is **not merged**, cleanup **warns** that the unmerged commits will be lost, names the branch, and **requires confirmation** (or `--yes`) before deleting. A user-owned worktree and branch (case 1a) are never touched. This is what makes the task branch genuinely durable (§2): it survives worktree removal, and `--cleanup` reclaims it only when the work is safely merged or the user explicitly confirms. Because the run *data* lives centrally, removing a worktree by any means never loses it. (O2's `--cleanup` implementation adopts this merge-aware behavior.)
...
- Case 1b: matching branch, non-dedicated cwd, **clean working tree** → the current checkout is switched to the default branch (or detached as fallback) and a worktree is created for the existing task branch; `createdByGan` true. A **dirty** working tree makes 1b refuse with guidance — no auto-stash or detach over uncommitted work.
...
- `--cleanup` always removes a run's central-store directory. For a gan-created workspace (1b/1c): the run-scoped worktree is removed; a **merged** task branch is deleted locally and on the remote; an **unmerged** branch triggers a warning (naming the branch) and is not deleted without confirmation or `--yes`. A user-owned (1a) worktree and branch are never touched.
