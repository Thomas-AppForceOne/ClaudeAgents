...
| `registerModule(moduleName, manifest)` | Probes the module registry to confirm a module is registered. Returns `{mutated: true, path}` if found, `{mutated: false, reason: "unknown-module:<name>"}` otherwise. Actual registration (manifest discovery + validation + prerequisite execution) happens lazily on first registry query; the `manifest` argument is currently advisory. The `pairsWith` invariant is enforced separately by `validateAll()`, not by this probe. |
