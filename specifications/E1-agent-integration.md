...
- **Snapshot freshness.** The snapshot is frozen for the entire `/gan` run, including across multi-sprint plans. The orchestrator does not re-snapshot between sprints; user edits to overlays or stack files mid-run are not picked up until the next `/gan` invocation (per F2). The one exception: when an agent's work itself mutates configuration through an API write function (`updateStackField`, `setOverlayField`, `setModuleState`), the orchestrator may re-snapshot before spawning the next agent so the new state is visible.
