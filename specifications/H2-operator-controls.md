...
> **F7 dependency (resolved split, not deferred).** [F7](F7-central-run-data-store-and-worktree-execution.md) relocates run *data* to the central, repo-keyed store, and the confinement hook (below) now reads `$GAN_RUN_DIR` rather than constructing `.gan-state/runs/<id>/` paths. The kill-switch-vs-central-store tension is **resolved here by splitting the control channel along F7's own logic**, rather than left as an open call: the **`halt` presence sentinel stays project-local** at the project's own `.gan-state/runs/<id>/control/halt` — a stable, hand-guessable path — because the kill-switch's entire value is that an operator can `touch` it by hand when the MCP server or CLI is wedged (the §"One deliberate exception" escape hatch), and F7's hashed central path (`<store-root>/<repo-key>/runs/<id>/`) is **not** hand-guessable, so relocating the sentinel there would defeat that escape hatch. The **recorded control state** — the steer-note *content* and the halt/steer entries in `progress.json` and the trace — follows run data into the F7 central store like every other durable artefact. The hook's halt check resolves the project-local sentinel relative to the **project root** (the implementing PR pins the derivation — e.g. the git top-level of the worktree; note `$GAN_WORKTREE` is *not* itself the project root in the run-scoped-worktree cases 1b/1c), independent of the hashed `$GAN_RUN_DIR`. The `.gan-state/runs/<id>/control/` paths below denote that project-local sentinel location; durable records read as the central store.
