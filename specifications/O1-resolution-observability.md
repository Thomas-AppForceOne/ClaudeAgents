...
O1 changes `gan config print`'s fail-open behaviour (it prints the partial resolved config — the flat `getResolvedConfig()` shape, whose own `issues`/`warnings` arrays carry the validation results — even when validation fails) and its exit-code policy — a CLI/server (installed-package) change that takes effect only via `install.sh`'s version-gated `npm install -g .`. Per the pre-1.0 install-version bump discipline (roadmap § "Pre-release chores and release gate"), O1's implementation PR **minor-bumps `package.json` `version`** (`0.MINOR.0`). (The `--print-config` flag parsing and the startup-log line live in `SKILL.md`, copied every install; the bump is for the `gan config print` package change.) **O3 ships in the same slot (21) and also minor-bumps** — if O1 and O3 land in one PR that is a single coordinated bump; if separate PRs, the second out-numbers the first.
