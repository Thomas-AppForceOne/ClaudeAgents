...
- a port the container binds → registered via `dockerReservePort`. **Port semantics (confirm against R7):** the shipped `PortRegistry.register(worktreePath, port, containerName)` takes a **caller-supplied** port and throws `PortInUse` if another worktree already holds it (`src/modules/docker/PortRegistry.ts:92,103`) — it is *collision-detecting*, not a free-port *allocator*. So the generator selects a candidate port (pre-checking with `PortValidator.isPortFree` where useful) and registers it, retrying another candidate on `PortInUse`; "collision-free across concurrent runs" comes from that registry check (backed by the F8 module-state store), not from an allocator. Whether R7's `dockerReservePort` is a thin wrapper over `register` (caller supplies the port) or composes `PortValidator` + a candidate scan into a true allocator is an **open R7-tool-semantics question** the integrity pass must pin — M4 must not assert allocation the substrate does not provide;
...
- **R7** — exposes the docker tools the rewrite calls (`dockerReservePort`/`dockerDiscoverPort`/`dockerContainerName`). Must land first. (The precise `dockerReservePort` allocate-vs-register semantics are an open seam the integrity pass reconciles — see Proposed change.)
