...
| `--reset-attempts` | A1 | Modifier valid only alongside `--recover`; standalone use is an error (the rejection mechanics land with the recovery work). When set, the recovered sprint resumes with attempt counters at zero. **Without** it, recovery preserves the attempt counters reconstructed from the trace, so a recovered sprint that hits the same loop halts again on the next attempt. | n/a — modifier. |
