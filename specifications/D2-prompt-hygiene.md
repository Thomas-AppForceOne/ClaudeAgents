...
- **The six agent prompts repeat boilerplate:** the snapshot-is-data bullet (in the agents that receive a snapshot — the proposer's variant **already differs**, so extraction must *normalize* an existing divergence, not assume byte-identity), the "do not call the Configuration API yourself" rule (the *concept*, ~6×; the literal-string count is lower), a near-identical `## Errors` section (6 copies), the `code/file/field/line/message` field list (6 copies — there are exactly six agent files), and "do not reference ecosystem-specific tools by name". Counts are approximate; the dedup target is real.
...
The boilerplate repeated across all six agents — the snapshot-is-data framing, "do not call the Configuration API yourself", the `## Errors` structured-error discipline, "do not reference ecosystem-specific tools by name", and the confinement paragraph — is factored into **one canonical source partial** (`agents/_house-rules.md`). These are **universal** rules: they govern *how* an agent handles inputs and writes, not *what* it does — so they apply to **every** agent, including E8's new contract-free `gan-reviewer-independent` (whose distinctive fresh-context, contract-free framing lives in its role-specific body, not the shared block). Byte-identical parity therefore holds for all agents without conflicting with the reviewer's independence.
...
- **No hard predecessor — D2 lands first.** D2 consumes nothing the other unimplemented specs produce: it de-verboses the *current* `SKILL.md` + six agents, factors the shared preamble, and installs the lints — so it has no hard dependency on R7 or anything downstream (only on shipped E1/R4, above). **R7, E8, D1, and the O-series all land *after* D2 and conform to its format**: R7's worktree-rewrite + trace-integration edits, E8's renegotiation loop and new reviewer prompt, D1's status markers, and the O-series surfaces are all auto-gated by D2's `agents/_house-rules.md` parity check and `lint-no-spec-ref`, with the release-gate hygiene re-check catching residual prose. D2 leads because its gains compound — a lean, lint-enforced `SKILL.md` benefits every subsequent spec and every interim dogfood run, and every later editor adds to the clean format from the start rather than being cleaned up afterward. (R7 stays the functional keystone; D2 leading only means the prompt format is set before R7's own `SKILL.md` edits land.)
