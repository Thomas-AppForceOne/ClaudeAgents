...
This is what E8 relies on (overlay `renegotiationCap`, `run-trace` `contractRevision`, both additive-in-place) and what Q2's evidence-bundle field (v1.1) will. **This fold is a blocking prerequisite for E8 (and R7/O2/O3), not a loose flag.** PROJECT_CONTEXT § "Schema discipline" (≈ line 130) still says *"pre-v1.0, any schema change bumps `schemaVersion`"*, reinforced by the Don'ts (≈ lines 196–197: *"Any additive change still bumps the version pre-1.0"*) — which **directly forbids** the additive-in-place edits E8/R7/O2/O3 require: an implementer obeying the authority doc would bump `run-trace`→v2 and `overlay`→v2, breaking E8's hard requirement and the shipped `evaluator-evidence-bundle-v1` join key. Per the single-writer rule **only the spec-validator edits PROJECT_CONTEXT**, so **before E8 merges** the spec-validator MUST (a) replace that pre-v1.0 clause with this additive-stays-`vN` ruling, and (b) delete the contradicting pre-1.0 sentences in the Don'ts. The roadmap records the ruling; PROJECT_CONTEXT is its permanent home — but the fold **gates E8**, tracked here, not deferred indefinitely.
...
**Effort estimate:** ~32–42 sprints of focused work (~8–11 calendar months at one full-time developer). The two largest new specs — R7 (runtime bridge) and E8 (independent review + forced verification) — are roughly a third of that and sit on the critical path (M4, the third new spec, is a small off-critical-path prompt rewrite); the overlay-UX polish (U1/U2/U3) moves to v1.1 to make room for them.
