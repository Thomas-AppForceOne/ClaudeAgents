...
- **Self-contained install (copies, not symlinks; build-on-install; bin verification).** `install.sh` copies `agents/*.md` and `skills/gan/` into `~/.claude/` as real files / directories rather than symlinking back to the source repo. Once install completes, the source repo can be moved or deleted without breaking the install. `package.json` declares a `prepare` script (`"prepare": "npm run build"`) so `npm install -g .` automatically compiles `dist/` before linking the bin — without it, npm "succeeds" while skipping the bin shim because the target file is absent. `install.sh` runs a post-install verification: if `command -v claudeagents-config-server` is empty after the npm step, the installer halts with a structured error and rolls back rather than reporting success on a setup that cannot run. R2 spec updated to match. Rationale: the original symlink-based install coupled the install to the source-repo path forever; the missing `prepare` script masked install failures behind a misleading success message. Both regressions bit a real dogfooding user.

- **MCP registration uses the absolute path to the bin.** `install.sh` resolves `claudeagents-config-server` via `command -v` at install time and writes the absolute path (e.g. `/opt/homebrew/bin/claudeagents-config-server`) into `~/.claude.json`'s `mcpServers.claudeagents-config.command` field. The original implementation wrote the bare command name, which fails on macOS because GUI-launched apps (Claude Code from Dock / Spotlight / Finder) inherit a minimal PATH that doesn't include `/opt/homebrew/bin/`, npm's prefix, or any shell-set additions. Symptom: install reports success, MCP appears registered in `~/.claude.json`, but `/gan --print-config` fails with `ConfigApiUnreachable` because Claude Code's spawn fails to resolve the bare command. R2 spec updated to require absolute-path registration.

- **Symmetric uninstall and post-uninstall restart hint.** `install.sh --uninstall` runs `npm uninstall -g @claudeagents/config-server` after removing filesystem state, so `gan` and `claudeagents-config-server` are no longer on PATH after uninstall. The original behaviour left the npm package in place under a (bogus) "other tools may depend on it" rationale; the package is namespaced and ships only the framework's two bins, so nothing external can depend on it. Failure of `npm uninstall` (permissions, network, npm absent) emits a warning but does not abort — filesystem cleanup is the load-bearing part. The success message also names the post-uninstall restart requirement: "Restart Claude Code to remove `/gan` from the slash-command palette." Mirrors the post-install restart hint and prevents the "I uninstalled but `/gan` still shows up" confusion.

- **`gan stacks --help` advertises every subcommand.** The CLI ships six `stacks` subcommands (`list`, `available`, `new`, `where`, `customize`, `reset`) but `gan stacks --help` originally advertised only `list` and `new`. A user running `gan stacks list` and seeing only `generic` had no obvious path to discover that `gan stacks available` lists every stack the framework ships. Help text now lists all six subcommands and explicitly distinguishes `list` (active for current dir) from `available` (everything on disk). Help-text-only fix; `gan stacks list` runtime output is unchanged for scripting compatibility.

- **Diagnostic-message clarity: distinguish "framework not installed" from "Claude Code needs restart."** The orchestrator's `ConfigApiUnreachable` preflight currently emits a single remediation: "Install the framework: `bash .../install.sh`. Then restart Claude Code." For a user who already ran `install.sh`, this remediation is misleading — they don't need to re-install, they need to restart Claude Code so the live session picks up the MCP registration. Diagnostic should branch: when `~/.claude.json` already contains the `mcpServers.claudeagents-config` entry pointing at an existing bin, surface "Framework is installed but Claude Code's running session has not loaded the MCP registration yet. Quit Claude Code completely (Cmd+Q) and reopen, then re-run `/gan --print-config`." When neither condition holds, the existing install-then-restart remediation applies. Sharper diagnostics save users a redundant install cycle.

#### Findings from the first dogfooding session

The chores below were authored against the first real `/gan` dogfooding session (a user authoring a project-tier overlay for a non-Node project, then approving trust, then trying to run `/gan`). Each row names the symptom the user hit, the framework gap, and the fix scope. All eight are v1.0 work because they bit a real user on first contact.

- **Permission-allowlist consent flow during install.** `install.sh` adds a `configure_permissions` step after prerequisite checks: an interactive 8-category prompt (Framework MCP / file ops / sub-agents / git read / git write / build & test / dep install / filesystem helpers) where the user approves or declines each. Single-keypress shortcuts: `[a]` approve all remaining, `[s]` skip all remaining, `[v]` expand a category to its literal allow entries before deciding. Approved categories are merged into `~/.claude/settings.json` `permissions.allow` via the same atomic-write pattern used for `~/.claude.json`. Non-TTY contexts default to minimal (framework MCP only); `--approve-all-permissions` and `--minimal-permissions` flags override either way. Re-runs skip already-granted categories; `--reconfigure-permissions` forces a re-prompt. Uninstall removes only the entries the framework added (surgical, leaves user-authored entries alone). Closes the dogfooding friction where every framework MCP call triggered a permission prompt mid-sprint, and matches the framework's existing trust-prompt consent pattern.

- **MCP-server cache invalidation on state-changing writes.** After `trustApprove` lands and after overlay-file mtime advances, the resolver's internal data caches must invalidate. The dogfooding bug: a user wrote `.claude/gan/project.md`, called `trustApprove`, and `getResolvedConfig` continued returning pre-overlay state for the rest of the Claude Code session — only a Claude Code restart picked up the new overlay. The framework's snapshot-freshness rule (re-snapshot on `mutated:true`) covers the orchestrator's view; it does NOT cover the server's internal caches. F2 gains a cache-coherence contract: every state-mutating tool call (write tools, `trustApprove`, `trustRevoke`) invalidates the resolver's data caches before returning. Mtime-driven invalidation handles the case where the user edits a file by hand. R1's implementation lands the actual cache bump.

- **Drop `NotImplemented` tools from the advertised tool list.** `getOverlayField` and `getStackConventions` ship as `NotImplemented` stubs but appear in the MCP `tools/list` response. Listing tools that always throw is worse than not listing them — the agent reads the schema, calls the tool, gets `NotImplemented`, and has no signal that the tool was never real in this version. One-line filter in `buildToolList()` excludes any tool whose dispatch path is `NotImplemented`. When the tools land in v1.1, they re-appear automatically.

- **Trust prompt: agent-callable, documented as protocol not gate.** `trustApprove` is a regular MCP tool the agent can call directly; the "trust prompt" UX described in F4 is a protocol the orchestrator is supposed to honor, not a gate the server enforces. F4 currently reads as if the prompt is a hard gate. Documentation fix in v1.0: F4 explicitly states `trustApprove` is agent-callable and the orchestrator's faithful surfacing of the diff before calling is part of the threat model's trusted base. Server-side enforcement (rate-limiting, content-hash echoes, structured audit log) is a v1.1 F4 amendment with options laid out then. The v1.0 fix is truth-in-advertising, not enforcement.

- **SKILL.md status markers for shipped vs deferred behavior.** The orchestrator skill spec describes `--recover` and `--list-recoverable` dispatching to "the recovery flow per O2's revision," but O2 is not implemented in v1.0. An orchestrator implementing the skill from scratch has no signal that those flags are aspirational. Each contract section in SKILL.md gets a status marker: `[shipped-in-v1.0]` (operative now), `[deferred-to-v1.1]` (described for forward-compat, not yet operative), `[partial-v1.0]` (minimal viable shipped, full version in v1.1). Flags that dispatch to deferred features print "this command requires v1.1; install latest framework" and exit non-zero rather than silently no-op. Same discipline applies to A1, T1, E5 as they ship in stages.

- **`stack.override` silent-shrinkage warning.** When a project-tier overlay's `stack.override` produces an active set strictly smaller than the detection-set ∪ override (i.e. the override suppresses one or more stacks that detection would otherwise activate), the resolver emits a structured warning naming the suppressed stacks. C2 spec amended to require the warning. O1's startup log surfaces it; `gan stacks list` annotates `active = […]` with `detection-only = [...] (suppressed by stack.override)` so the user sees what they lost. The dogfooding bug: a user authored a `php-grav` stack with prose describing coexistence with `web-node`, but `stack.override: [php-grav]` silently dropped `web-node`. Their prose became a lie with no warning.

- **Framework-owned confinement hook.** Today the `gan-confine.sh` hook lives in each project's `.claude/hooks/` and references the framework's filesystem zones directly. When F1 reworked zones (`.gan/` → `.gan-state/`), every project that adopted the hook pattern silently broke until manually updated. The framework should own the hook: `install.sh` writes a user-tier hook at `~/.claude/hooks/gan-confine.sh` that the framework maintains and updates on each install. Project-tier hooks remain optional overrides. Closes the implicit cross-version coupling where filesystem-zone changes leaked into every downstream project.

- **`gan stacks new --tier=project` template fix.** The scaffold currently includes a `detection:` block in project-tier files, even though project-tier files reject `detection:` (they must use `stack.override` instead). A user copying from `web-node.md` to author a project-tier customisation trips the validator on first run. R3's scaffold becomes tier-aware: project-tier scaffolds omit `detection:` and include a comment pointing at `stack.override`; user-tier scaffolds include `detection:` (user-tier files can declare detection patterns). Tiny fix; closes a first-use trap.
