...
- **[Q5](Q5-documentation-quality-enforcement.md)** — documentation-quality enforcement. Two new stack fields source a project's documentation standard from config (`documentationSurfaces`, instantiated as gating contract criteria via C1's template-instantiation protocol exactly like `securitySurfaces`; `docLintCmd`, a deterministic baseline-relative doc-lint run by evaluator-core). Three enforcement layers — default stack convention, deterministic lint, evaluator-scored gating criteria — with the per-rule gates-vs-warns split living on the deterministic layer. Supersedes/extends C1 (schema), E1/E2 (proposer + evaluator prompts), and E3 (evaluator-core) via the new-spec mechanism; those shipped specs are not edited. Default-standard wording/thresholds tuned against v1.0/v1.1 dogfooding traces (release-driven). Depends on C1, C2, C3, C5, E1/E2, E3, T1; shares the quality-signal slice with Q1.
