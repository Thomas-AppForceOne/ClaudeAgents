...
This is what E8 relies on (overlay `renegotiationCap`, `run-trace` `contractRevision`, both additive-in-place) and what Q2's evidence-bundle field (v1.1) will. **Flagged for the spec-validator** to fold this clarification into PROJECT_CONTEXT § "Schema discipline" under the single-writer rule — the roadmap records the ruling; PROJECT_CONTEXT remains its permanent home.
...
16. **Next. [R7](R7-runtime-invocation-bridge.md) — runtime invocation bridge.** ~3–4 sprints. **The keystone.** Because the orchestrator and agents run as markdown executed by Claude, they can call only what is exposed as an MCP tool or `gan` CLI subcommand — which today is the config server alone. The `trace`, `safety`, and `evaluator-core` libraries (T1, A1, E3) and the module helpers (M2 docker) are shipped, tested, and **uncallable from a markdown orchestrator**: real runs emit zero trace events, no loop-detection halt can fire, the evaluator cannot consume a deterministic plan, and the generator hand-rolls Docker instead of calling `PortRegistry`. R7 exposes those four library surfaces — plus the run-store resolution layer (F7) that *produces* the `runDir`/`repoKey` they all address by, the keystone without which none of the others is even callable — to the orchestrator and agents via MCP tools and/or `gan` shims, delegating to the already-tested functions. It does not edit the shipped T1/A1/E3/M2 specs — it is the cross-reference that makes them operative. Everything below depends on it.
...
18. **Docker module wiring.** ~1–2 sprints. Folds into R7's module arm + E8 as **two prompt rewrites with named owners** (so neither falls through the gap): (a) the **generator-side is this slot's own deliverable** — it rewrites `agents/gan-generator.md` so a docker-active stack calls R7's `dockerReservePort` / `dockerDiscoverPort` / `dockerContainerName` tools instead of inventing port logic. No separate spec file: the rewrite rides this slot, because R7 only *exposes* the tools (it does not decide call sites) and D2 is behaviour-preserving (it won't add the call either). (b) The **evaluator-side is owned by E8** (§3 "Forced deterministic verification") — it gates the relevant criterion on real `ContainerHealth` via R7's `dockerCheckContainerHealth` ("the container actually boots"). Reconciles M2's explicitly-provisional API against a real agent call-graph — the existing Docker dogfood project is that call-graph. Depends on R7.
19. **[D1](D1-diagnostic-clarity.md)** — diagnostic clarity + SKILL.md status markers. ~2–3 sprints. Re-scoped: the "which sections are operative" audit runs **after** R7, so the markers reflect a bridge-wired runtime rather than aspirational prose. **D1's `--recover` / `--list-recoverable` marker ACs additionally depend on O2 (slot 20):** D1 is co-grouped with O2 ({D1, O2→O3} below), so O2's recovery flow lands before those specific markers are validated (the recovery-dispatch AC rides with O2 if D1 lands first) — without that edge, D1 would mark `--recover` `[partial-v1.0]` against a flow O2 has not yet wired, the exact drift D1 exists to catch.
20. **[O2](O2-recovery.md) (minimal recovery).** ~1–2 sprints. `--recover` becomes meaningful only once R7 makes the trace real; ship minimal trace-driven resume — the `[partial-v1.0]` slice of O2 (see its status markers) — and defer the full recovery/cleanup UX to v1.1. Depends on R7. **Lands before O3** (slot 21), which reads O2's `terminalReason` enum and run-dir layout. **Note the E8↔O2 seam:** E8 (slot 17) writes `progress.json` fields (`failed-evaluation-rejected`, `contractRevision`) *before* O2 authors the strict `progress-v1.json` here, so O2's schema must absorb them — the integration test is O2's AC 29. Co-schedule the `progress-v1.json` schema slice with E8 if implementing out of order.
21. **[O1](O1-resolution-observability.md) + [O3](O3-telemetry-semantics.md) (minimal).** ~2 sprints. Full startup-log surface (O1) and the stderr cost/progress fuel-gauge (O3) — now backed by real trace data from R7. O3 depends on O2 (slot 20) for the `terminalReason` enum (incl. E8's `failed-evaluation-rejected`); O1 is independent.
22. **[D2](D2-prompt-hygiene.md) — prompt hygiene.** ~1–2 sprints. **Lands last among SKILL.md editors** (after E8, D1, O2, O1/O3) so it de-verboses the *final* v1.0 SKILL.md, not a version a later spec re-bloats: collapse the three near-identical safety sections into one halt contract, drop self-quotation, factor the boilerplate repeated across all six agent prompts into one shared preamble (build-time assembly, not runtime injection), and strip internal `specifications/*` cross-references (the `lint-no-spec-ref` backstop, distinct from `lint-no-stack-leak`). Pure clarity/precision; no behavioural change. Depends on E8 + D1 + the O-series SKILL.md additions.
23. **Pre-release chores + release gate.** See below.

Dependencies within slots 19–22 are real, not free parallelism: **O2 (20) → O3 (in 21)** for the `terminalReason` enum and run-dir layout; **D2 (22) lands after every other SKILL.md editor** (E8 17, D1 19, O2 20, O1/O3 21) — the hygiene pass must be the *final* edit to SKILL.md, since its whole point (lowering execution-fidelity risk) is defeated if a later spec re-bloats the file. So the shape after R7 (16) is **R7 → E8 → {D1, O2 → O3} → D2**, with O1 and the Docker wiring (18) independent. E8 (17) is the critical path. The post-v1.0 dogfooding audit fires after slot 23.
...
- **External-proof release gate — the single highest-risk, least-automatable item; a resourced workstream, not a chore.** Because the orchestrator is markdown-run-by-Claude and CI has no LLM, **no automated test proves the end-to-end loop works** (see Known gaps). The release therefore gates on a **structured dogfood protocol**, planned and resourced from the *start* of the release, not discovered at the freeze:
