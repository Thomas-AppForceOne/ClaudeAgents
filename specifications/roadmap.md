...
| `agents/gan-evaluator.md` | E1 | `M` — rewritten in place. The hardcoded stack-specific tokens (`kt`, `kts`, `gradle`, `npm audit`, etc.) move into `stacks/<name>.md` per E2; the rewritten prompt contains zero stack-specific tokens, verified by R4's `lint-no-stack-leak`. |
