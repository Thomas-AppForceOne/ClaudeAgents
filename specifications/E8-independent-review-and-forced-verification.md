...
- **The renegotiation cap is accounted *separately* from A1's sprint budget — they do not share a counter.** This is the concrete fix for the wrong-halt hazard, not a deferred tuning note. A1's sprint budget (`n × roleCount + 4`, default ~8–10 per `src/safety/config.ts`, with `roleCount` = multi-attempt roles only) guards thrash *within a single contract revision*. The E8 renegotiation cap guards thrash *across revisions*. A renegotiation round **resets the per-revision attempt accounting** A1's budget sees, so re-running proposer/generator/independent-reviewer/evaluator for a new revision cannot trip `sprintBudgetExceeded` before the cap is reached. Without this separation, a 2-round renegotiation (four roles × rounds) would exceed the default budget of ~8 and fire a `LoopDetected` halt *before* the cap — the exact outcome the next bullet forbids. So: A1's budget bounds one revision's attempts (genuine within-revision non-convergence still halts as `LoopDetected`); the cap bounds the number of revisions.
...
- **Contract revision artifacts:** the canonical `sprint-{N}-contract.json` (latest locked, unchanged join target) plus archived siblings `sprint-{N}-contract.r{k}.json`; the active revision index lives in `progress.json`. **No change to `evaluator-evidence-bundle-v1`** — `contractRevision` is run state in `progress.json`, not a bundle field — so E8 touches no shipped schema. (Flag for maintainers: the shipped `evaluator-evidence-bundle-v1` description's "additive stays v1" wording and PROJECT_CONTEXT's "pre-v1.0, *any* schema change bumps `schemaVersion`" disagree. E8 sidesteps the seam by not changing the bundle, but a future additive bundle change — e.g. Q2's out-of-contract field — must first resolve which convention governs. This is a convention defect to settle before the v1.0 freeze, independent of E8.)
...
- **Overlay schema change (the renegotiation cap is not a free addition).** The renegotiation cap lands in the overlay `safety.*` block — but that block (`schemas/overlay-v1.json`, `additionalProperties: false`) today allows exactly `{attemptCeilings, oscillationDetection, sprintBudget}`, so adding `renegotiationCap` **is a schema change**. Per PROJECT_CONTEXT's pre-v1.0 rule it bumps the version (overlay-v2 + migration tooling) — unless the maintainer ruling on the additive-vs-bump convention conflict (flagged above) lands first and permits an additive edit; the implementing PR resolves which. The **reviewer-model selection** is a per-role config knob (the C6 seed) and lands in [`runtime-knobs.md`](runtime-knobs.md). Both are listed here explicitly rather than buried, so the schema impact is visible at review time.
- **`progress.json` fields E8 writes (absorbed by O2's schema):** the `failed-evaluation-rejected` `terminalReason` value and the `contractRevision` field. E8 (slot 17) is the writer and ships before O2 (slot 21) authors the strict `progress-v1.json`; O2's schema must include both or E8-renegotiated runs fail validation. Cross-referenced in O2's Dependencies + AC 29. (`terminalReason` is a loose string in shipped code today, so E8 writes the value fine until O2's schema lands.)
