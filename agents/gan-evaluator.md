...
When any framework API call returns a structured error, record the affected criterion as `verdict: "blocked"` and place the F2 fields in its `evidence` preserved verbatim: `code`, `file`, `field`, `line`, `message`.
