...
- **Measurement is separate from gating.** Verification has two independent axes: what the framework MEASURES and what it GATES on. Every verification finding carries a severity (blocker / warning / advisory) and a gating policy: blockers fail the attempt; warnings are recorded in run state and surfaced; advisories route to the next generator attempt and/or spin into follow-up tasks but never block. The LLM evaluator's PASS/FAIL on contract criteria remains the SOLE authoritative gate — quality findings inform but never corrupt the adversarial signal. The advisory tier is the W-series non-aborting-warning philosophy (per W1) applied to generator output rather than user misuse. Future Q / V / A specs inherit this split rather than each re-deciding it.
- **Integrity probes default to delta/ratchet semantics.** Probes assessing codebase integrity (duplication, lint count, dead code, coverage gaps, dependency risk) default to BASELINE-RELATIVE measurement: did this change, against the run's base ref, make the metric worse? Absolute thresholds are unusable on brownfield repos; deltas port to any project age. Q1 (churn/revert) and Q3 (coverage delta) are instances; this rule generalises it so future integrity probes inherit delta-by-default. Any such probe still extends through E3's existing splice points (`evaluator.additionalChecks`, `securitySurfaces`) and stays a pure function over (file content, file path, sprint plan) or it leaves the deterministic core (per E3).
